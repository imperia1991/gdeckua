...
class Places extends ActiveRecord
{
	...
	public function rules()
	{
		...
		return [
			[
				'title_ru, address_ru, created_at, district_id, description_ru, short_description_ru',
				'required',
				'on'      => self::SCENARIO_RU,
				'message' => Yii::t('main', 'Необходимо заполнить поле «{attribute}»')
			],
			[
				'title_uk, address_uk, created_at, district_id, description_uk, short_description_uk',
				'required',
				'on'      => self::SCENARIO_UK,
				'message' => Yii::t('main', 'Необходимо заполнить поле «{attribute}»')
			],
			...
			[
				'user_id, updated_at, country_id, region_id, city_id, description_ru, description_uk, district_id, search, how_to_get_ru, how_to_get_uk, short_description_ru, short_description_uk, category_id',
				'safe'
			],
	 ...
	 * @return string
	 */
	public function getTitleWithAddress()
	{
	...
	{
		return Yii::app()->createUrl('/' . Yii::app()->getLanguage() . '/view/' . $this->id . '/' . $this->alias);
	}
	...
	{
		return 'р-н ' . $this->getDistrict() . ', ' . $this->getAddress();
	}
